...
async function initializeDatabase() {
  ...
  initPromise = (async () => {
    try {
      ...
      // Add lover_user_id column for promoting friends to lover status
      try {
        await sql`
          ALTER TABLE users
          ADD COLUMN IF NOT EXISTS lover_user_id INTEGER REFERENCES users(id) ON DELETE SET NULL
        `;
        console.log('Ensured lover_user_id column exists on users table');
      } catch (err) {
        if (!err.message.includes('already exists')) {
          console.error('Could not ensure lover_user_id column:', err.message);
        }
      }
