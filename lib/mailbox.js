...
async function validateWord(word) {
  ...
  // Also check if it's a valid English word using a simple pattern
  // (only letters, at least 2 characters)
  if (/^[a-z]{2,}$/.test(normalized)) {
    // For production, you might want to use an API like:
    // const response = await fetch(`https://api.dictionaryapi.dev/api/v2/entries/en/${normalized}`);
    // return response.ok;
    // For now, we'll accept any valid lowercase word with letters only
    return true;
  }
  return false;
}
...
async function getHangmanGame(gameId, userId) {
  ...
  const maskedWord = game.target_word
    .split('')
...
async function guessLetter(gameId, userId, letter) {
  ...
  if (isInWord) {
    ...
    const allRevealed = game.target_word
      .split('')
      .every(letter => newRevealedLetters.includes(letter));
...
async function guessWord(gameId, userId, word) {
  ...
  if (normalizedWord === game.target_word) {
    ...
    newRevealedLetters = game.target_word.split('').filter((v, i, a) => a.indexOf(v) === i);
    newStatus = 'won';
    await updateHangmanStats(userId, true);
  } else {
