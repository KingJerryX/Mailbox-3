...
async function saveCountdownTimer(userId, timerData) {
  ...
  const result = await sql`
    ...
    VALUES (${userId}, ${sender_id || null}, ${timer_name}, ${target_date}, ${target_time}, ${timezone}, ${enabled !== undefined ? enabled : true})
    RETURNING id
  `;

  return await getCountdownTimer(result.rows[0].id, userId);
}
