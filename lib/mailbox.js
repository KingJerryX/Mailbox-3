...
async function getCountdownTimer(userId) {
  await ensureDatabaseInitialized();
  const result = await sql`
...
async function saveCountdownTimer(userId, timerData) {
  ...
  const { timer_name, target_date, target_time, timezone, show_large, enabled } = timerData;

  // Check if timer exists
  const existing = await sql`
    SELECT id FROM countdown_timers WHERE user_id = ${userId} LIMIT 1
  `;
  ...
  if (existing.rows.length > 0) {
    ...
    // Insert new
    await sql`
      INSERT INTO countdown_timers (user_id, timer_name, target_date, target_time, timezone, show_large, enabled)
      VALUES (${userId}, ${timer_name}, ${target_date}, ${target_time}, ${timezone}, ${show_large}, ${enabled || true})
    `;
  }

  return await getCountdownTimer(userId);
}
