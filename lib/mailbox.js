...
async function updateBucketListItem(itemId, userId, updates) {
  ...
  if (title !== undefined) {
    updateParts.push(sql`title = ${title}`);
  }
  if (description !== undefined) {
    updateParts.push(sql`description = ${description}`);
  }
  if (category !== undefined) {
    updateParts.push(sql`category = ${category}`);
  }
  if (image_url !== undefined) {
    updateParts.push(sql`image_url = ${image_url}`);
  }
  if (isCompleted !== undefined) {
    updateParts.push(sql`is_completed = ${isCompleted}`);
    if (isCompleted) {
      updateParts.push(sql`completed_at = CURRENT_TIMESTAMP`);
    } else {
      updateParts.push(sql`completed_at = NULL`);
    }
  }
  ...
  // Execute update - combine all parts
  try {
    // Build the query manually since sql.join might have issues
    const setClause = updateParts.map((part, idx) => {
      // Extract the SQL and values from each part
      return part.strings[0].trim();
    }).join(', ');

    // For now, use separate queries which is more reliable
    if (isCompleted !== undefined) {
      if (isCompleted) {
        await sql`UPDATE bucket_list_items SET is_completed = TRUE, completed_at = CURRENT_TIMESTAMP WHERE id = ${itemId}`;
      } else {
        await sql`UPDATE bucket_list_items SET is_completed = FALSE, completed_at = NULL WHERE id = ${itemId}`;
      }
    }

    if (title !== undefined) {
      await sql`UPDATE bucket_list_items SET title = ${title} WHERE id = ${itemId}`;
    }
    if (description !== undefined) {
      await sql`UPDATE bucket_list_items SET description = ${description} WHERE id = ${itemId}`;
    }
    if (category !== undefined) {
      await sql`UPDATE bucket_list_items SET category = ${category} WHERE id = ${itemId}`;
    }
    if (image_url !== undefined) {
      await sql`UPDATE bucket_list_items SET image_url = ${image_url} WHERE id = ${itemId}`;
    }
  } catch (error) {
