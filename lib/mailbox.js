...
async function updateBucketListItem(itemId, userId, updates) {
  ...
  let updateQuery = sql`UPDATE bucket_list_items SET `;
  const updatesList = [];

  if (title !== undefined) {
    updatesList.push(sql`title = ${title}`);
  }
  if (description !== undefined) {
    updatesList.push(sql`description = ${description}`);
  }
  if (category !== undefined) {
    updatesList.push(sql`category = ${category}`);
  }
  if (image_url !== undefined) {
    updatesList.push(sql`image_url = ${image_url}`);
  }
  if (isCompleted !== undefined) {
    updatesList.push(sql`is_completed = ${isCompleted}`);
    if (isCompleted) {
      updatesList.push(sql`completed_at = CURRENT_TIMESTAMP`);
    } else {
      updatesList.push(sql`completed_at = NULL`);
    }
  }
